...
/**
 *
 * FileName: SendCouponResultEnum
 * Author:   zhangqiuying
 * Date:     2024/10/10 12:49
 * Description:
 */
public enum SendCouponResultEnum {
    ...
    SendCouponResultEnum(Integer code, Integer plateCode, String desc) {
        this.code = code;
        this.plateCode = plateCode;
        this.desc = desc;
    }

    public static SendCouponResultEnum getByPlateCode(Integer code) {
        if (code == null) {
            return ERROR;
        }
        for (SendCouponResultEnum sendCouponResultEnum : SendCouponResultEnum.values()) {
            if (sendCouponResultEnum.plateCode.equals(code)) {
                return sendCouponResultEnum;
            }
        }
        return ERROR;
    }
